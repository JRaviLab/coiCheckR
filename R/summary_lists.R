...
#'
#' @param object A `coi_report`, as returned by [check_coi()].
...
#' @export
summary.coi_report <- function(object, ...) {
  rows <- list()
  ...
  if (nrow(object$second_degree) > 0) {
    rows$second_degree <- tibble::tibble(
      ...
      parties = paste(unique(object$second_degree$linked_author), collapse = ", ")
    )
  }
  ...
  if (length(rows) == 0) {
    return(tibble::tibble(relation = character(), n = integer(), parties = character()))
  }
...
#'
#' Turns [check_coi_batch()]'s output into plain-text lines suitable
...
#'
#' @param batch Output of [check_coi_batch()].
#' @return A character vector, one line per candidate, e.g.
#'   `"Mark Davies MR -- 1 direct (Nizet V), 0 second-degree, 0 funding"`.
#' @export
format_coi_summary <- function(batch) {
